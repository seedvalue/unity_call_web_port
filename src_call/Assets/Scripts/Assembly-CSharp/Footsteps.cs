...
public class Footsteps : MonoBehaviour
{
	...
	public float defaultStepVol = 1f;
	...
	public float foliageRustleVol = 1f;
	...
	{
		if (!(FPSPlayerComponent.hitPoints > 1f) || (InputComponent.moveX == 0f && InputComponent.moveY == 0f))
		{
			return;
		}
		if (!FPSWalkerComponent.prone)
		{
			...
			{
				if (FPSWalkerComponent.inWater)
				{
					if (waterSounds.Length > 0)
					{
						footStepClip = waterSounds[Random.Range(0, waterSounds.Length)];
						if (!FPSWalkerComponent.holdingBreath)
						{
							volumeAmt = waterSoundVol;
						}
						else
						{
							volumeAmt = waterSoundVol / 2f;
						}
				...
				else
				{
					...
					else if ((bool)Terrain.activeTerrain)
					{
						terrainIndex = GetTerrainTextureIndex(base.transform.position);
						if (terrainIndex == dirtTerrainIndex1 || terrainIndex == dirtTerrainIndex2 || terrainIndex == dirtTerrainIndex3)
						{
							if (dirtSteps.Length > 0)
							{
								footStepClip = dirtSteps[Random.Range(0, dirtSteps.Length)];
						...
						else if (terrainIndex == stoneTerrainIndex1 || terrainIndex == stoneTerrainIndex2 || terrainIndex == stoneTerrainIndex3)
						{
							if (stoneSteps.Length > 0)
							{
								footStepClip = stoneSteps[Random.Range(0, stoneSteps.Length)];
						...
						else if (terrainIndex == grassTerrainIndex1 || terrainIndex == grassTerrainIndex2 || terrainIndex == grassTerrainIndex3)
						{
							if (grassSteps.Length > 0)
							{
								footStepClip = grassSteps[Random.Range(0, grassSteps.Length)];
						...
						else if (terrainIndex == gravelTerrainIndex1 || terrainIndex == gravelTerrainIndex2 || terrainIndex == gravelTerrainIndex3)
						{
							if (gravelSteps.Length > 0)
							{
								footStepClip = gravelSteps[Random.Range(0, gravelSteps.Length)];
						...
						else if (terrainIndex == sandTerrainIndex1 || terrainIndex == sandTerrainIndex2 || terrainIndex == sandTerrainIndex3)
						{
							if (sandSteps.Length > 0)
							{
								footStepClip = sandSteps[Random.Range(0, sandSteps.Length)];
			...
			else if (climbSounds.Length > 0 && !FPSWalkerComponent.noClimbingSfx)
			{
	...
	{
		float[] texturesAtPoint = GetTexturesAtPoint(WorldPos);
		float num = 0f;
		int result = 0;
		for (int i = 0; i < texturesAtPoint.Length; i++)
		{
			if (texturesAtPoint[i] > num)
			{
				result = i;
				num = texturesAtPoint[i];
			}
		}
		return result;
	}
