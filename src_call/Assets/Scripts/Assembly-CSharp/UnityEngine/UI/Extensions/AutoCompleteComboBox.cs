...
namespace UnityEngine.UI.Extensions
{
	...
	public class AutoCompleteComboBox : MonoBehaviour
	{
		...
		{
			List<string> second = _panelItems.Where((string x) => x.ToLower().Contains(currText.ToLower())).ToList();
			List<string> list = _panelItems.Except(second).ToList();
			foreach (string item in list)
			{
				panelObjects[item].SetActive(false);
				_panelItems.Remove(item);
				_prunedPanelItems.Add(item);
			}
			List<string> list2 = _prunedPanelItems.Where((string x) => x.ToLower().Contains(currText)).ToList();
			foreach (string item2 in list2)
			{
				panelObjects[item2].SetActive(true);
				_panelItems.Add(item2);
				_prunedPanelItems.Remove(item2);
			}
		}
	}
}
