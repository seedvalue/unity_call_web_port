...
namespace UnityEngine.UI.Extensions
{
	...
	public class ImageExtended : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
	{
		...
		public override Texture mainTexture
		{
			get
			{
				return (!(overrideSprite == null)) ? overrideSprite.texture : Graphic.s_WhiteTexture;
			}
		}
		...
		private Vector4 GetDrawingDimensions(bool shouldPreserveAspect)
		{
			...
			Vector4 vector3 = ((!(overrideSprite == null)) ? new Vector4(vector.x / (float)num, vector.y / (float)num2, ((float)num - vector.z) / (float)num, ((float)num2 - vector.w) / (float)num2) : new Vector4(0f, 0f, 1f, 1f));
		...
		private void GenerateTiledSprite(List<UIVertex> vbo)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				s_UVTiled[0] = GetRotatedUV(vector2, 0, (i != 0) ? vector2 : vector, (i == 0) ? 1 : 3);
				s_UVTiled[1] = GetRotatedUV(vector2, 2, (i != 0) ? vector : vector2, (i == 0) ? 1 : 3);
			...
			for (int j = 0; j < 2; j++)
			{
				...
				s_UVTiled[0] = GetRotatedUV((j != 0) ? vector2 : vector, (j != 0) ? 2 : 0, vector2, 1);
				s_UVTiled[1] = GetRotatedUV((j != 0) ? vector : vector2, (j != 0) ? 2 : 0, vector2, 3);
				RotatePairUV(s_UVTiled);
			...
			if (m_FillAmount < 1f)
			{
				...
				if (fillMethod == FillMethod.Radial180)
				{
					for (int j = 0; j < 2; j++)
					{
						...
						s_Uv[2].y = s_Uv[1].y;
						s_Uv[3].y = s_Uv[0].y;
						float value = ((!m_FillClockwise) ? (m_FillAmount * 2f - (float)(1 - j)) : (fillAmount * 2f - (float)j));
				...
				if (fillMethod == FillMethod.Radial360)
				{
					for (int l = 0; l < 4; l++)
					{
						...
						s_Uv[2].y = s_Uv[1].y;
						s_Uv[3].y = s_Uv[0].y;
						float value2 = ((!m_FillClockwise) ? (m_FillAmount * 4f - (float)(3 - (l + m_FillOrigin) % 4)) : (m_FillAmount * 4f - (float)((l + m_FillOrigin) % 4)));
