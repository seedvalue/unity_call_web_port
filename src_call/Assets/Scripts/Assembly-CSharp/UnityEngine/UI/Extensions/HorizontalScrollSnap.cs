...
namespace UnityEngine.UI.Extensions
{
	...
	public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler
	{
		...
		private void Start()
		{
			...
			_scroll_rect.horizontalNormalizedPosition = (float)(_currentScreen - 1) / (float)(_screens - 1);
		...
		private void DistributePages()
		{
			...
			if (_screens > 0)
			{
				for (float num4 = 0f; num4 < (float)_screens; num4 += 1f)
				{
					_scroll_rect.horizontalNormalizedPosition = num4 / (float)(_screens - 1);
		...
		public void RemoveChild(int index, out GameObject ChildRemoved)
		{
			...
			_scroll_rect.horizontalNormalizedPosition = 0f;
			Transform transform = _screensContainer.transform;
			int num = 0;
		...
		public void OnEndDrag(PointerEventData eventData)
		{
			...
			if (UseFastSwipe)
			{
				...
				if (_fastSwipeCounter <= _fastSwipeTarget && Math.Abs(_startPosition.x - _screensContainer.localPosition.x) > (float)FastSwipeThreshold)
				{
					fastSwipe = true;
				}
				if (fastSwipe)
