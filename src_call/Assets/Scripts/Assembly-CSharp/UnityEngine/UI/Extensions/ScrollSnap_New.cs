...
namespace UnityEngine.UI.Extensions
{
	...
	[ExecuteInEditMode]
	public class ScrollSnap_New : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler
	{
		...
		private void Awake()
		{
			...
			if ((bool)TextButton)
			{
				TextButton.GetComponent<Button>().onClick.AddListener(delegate
				{
				});
			}
		}
		...
		public void UpdateListItemsSize()
		{
			...
			if (direction == ScrollDirection.Horizontal)
			{
				foreach (object item in listContainerTransform)
				{
					GameObject gameObject = ((Transform)item).gameObject;
			...
			foreach (object item2 in listContainerTransform)
			{
				GameObject gameObject2 = ((Transform)item2).gameObject;
		...
		public void UpdateListItemPositions()
		{
			...
			foreach (object item in listContainerTransform)
			{
				if (((Transform)item).gameObject.activeInHierarchy)
				{
					num++;
				}
			}
		...
		public void TestScreen(int i)
		{
			UpdateListItemPositions();
			if (i < pages)
			{
				lerp = true;
				lerpTarget = pageAnchorPositions[i];
				PageChanged(i);
			}
			else if (i > pages)
			{
