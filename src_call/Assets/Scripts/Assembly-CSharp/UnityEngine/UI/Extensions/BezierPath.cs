		...
		public void Interpolate(List<Vector2> segmentPoints, float scale)
		{
			...
			for (int i = 0; i < segmentPoints.Count; i++)
			{
				if (i == 0)
				{
					...
					Vector2 item = vector + scale * vector3;
				...
				else if (i == segmentPoints.Count - 1)
				{
					...
					Vector2 item2 = vector5 - scale * vector6;
					controlPoints.Add(item2);
					controlPoints.Add(vector5);
				}
				else
				{
					Vector2 vector7 = segmentPoints[i - 1];
					Vector2 vector8 = segmentPoints[i];
					Vector2 vector9 = segmentPoints[i + 1];
					Vector2 normalized = (vector9 - vector7).normalized;
					Vector2 item3 = vector8 - scale * normalized * (vector8 - vector7).magnitude;
					Vector2 item4 = vector8 + scale * normalized * (vector9 - vector8).magnitude;
