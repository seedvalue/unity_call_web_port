...
{
	private static List<Transform> npc_handlers = new List<Transform>();
	...
	private void Start()
	{
		...
		npc_handlers.Add(base.transform);
	}

	private void OnDestroy()
	{
		npc_handlers.Remove(base.transform);
	}
	...
	private void OnGUI()
	{
		if (active_target)
		{
			...
			Vector2 vector2 = handler_style.CalcSize(new GUIContent(NPC_Name));
			GUI.Label(new Rect(vector.x - vector2.x / 2f, (float)Screen.height - vector.y - vector2.y, vector2.x, vector2.y * 2f), NPC_Name);
		}
	}
	...
	private void Update()
	{
		...
		if (ID != npc_handlers.IndexOf(base.transform))
		{
			ID = npc_handlers.IndexOf(base.transform);
			base.name = "NPCObj" + ID;
		}
	}
